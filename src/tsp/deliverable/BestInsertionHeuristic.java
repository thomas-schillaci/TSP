...
/** 
 * The best insertion chooses a first city randomly, the second city as the nearest from the first one.
 * Then at each step it finds the best association of city and position of insertion to minimize the added length.
 * It is greedy.
 * @author Charles
 *
 */
public class BestInsertionHeuristic extends AHeuristic{
	...
	public void solve() throws Exception {
		...
		int secondCity =findNearestNeighbor(firstCity-1,unused);	//we choose the nearest city to the first one as second city
		used.add(secondCity);
		unused.remove(secondCity);
	...
	public int[] bestInsertion(ArrayList<Integer> used,ArrayList<Integer> unused) throws Exception{
		...
		int position=0;
		long addedLength=m_instance.getDistances(used.get(0),unused.get(0))+m_instance.getDistances(unused.get(0),used.get(1))-m_instance.getDistances(used.get(0),used.get(1));
		for (int unusedCity : unused) {
			for (int i=0;i<used.size()-1;i++) {
				if (addedLength>m_instance.getDistances(i, unusedCity)+m_instance.getDistances(unusedCity, i+1)-m_instance.getDistances(i, i+1)) {
					addedLength=m_instance.getDistances(i, unusedCity)+m_instance.getDistances(unusedCity, i+1)-m_instance.getDistances(i, i+1);
					city=unusedCity;
					position=i+1;
				}
			}
